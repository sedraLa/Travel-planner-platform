<x-app-layout>
        ...
        <form action="{{ route('activities.update', $activity->id  ) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="first-section">
                <div class="left">
                    ...
                    <x-text-input id="description" type="text" name="description" :value="old('description', $activity->description)"
                    ...
                    <select id="duration_unit" name="duration_unit"
                        class="block w-full border-gray-300 rounded-md shadow-sm" required>
                        <option value="minutes" {{ old('duration_unit', $activity->duration_unit) == 'minutes' ? 'selected' : '' }}>Minutes</option>
                        <option value="hours" {{ old('duration_unit', $activity->duration_unit) == 'hours' ? 'selected' : '' }}>Hours</option>
                        <option value="days" {{ old('duration_unit', $activity->duration_unit) == 'days' ? 'selected' : '' }}>Days</option>
                    </select>

                    <x-input-label for="start_time" value="Start Time" />
                    <x-text-input id="start_time" type="time" name="start_time" :value="old('start_time', $activity->start_time)" />

                    <x-input-label for="end_time" value="End Time" />
                    <x-text-input id="end_time" type="time" name="end_time" :value="old('end_time', $activity->end_time)" />

                    <x-input-label for="start_date" value="Start Date" />
                    <x-text-input id="start_date" type="date" name="start_date" :value="old('start_date', $activity->start_date)" />

                    <x-input-label for="end_date" value="End Date" />
                    <x-text-input id="end_date" type="date" name="end_date" :value="old('end_date', $activity->end_date)" />
                        <x-input-label for="availability" value=" Availability" />
                    <select id="availability" name="availability"
                        class="block w-full border-gray-300 rounded-md shadow-sm" required>
                        <option value="Available" {{ old('availability', $activity->availability) == 'Available' ? 'selected' : '' }}>Available</option>
                    ...
                    <x-text-input id="guide_name" type="text" name="guide_name" :value="old('guide_name', $activity->guide_name)" />

                    <x-input-label for="guide_language" value="Guide Language" />
                    <x-text-input id="guide_language" type="text" name="guide_language" :value="old('guide_language', $activity->guide_language)" />
                ...
                <div class="right">
                    ...
                    <select id="category" name="category" class="block w-full border-gray-300 rounded-md shadow-sm" required>
                        ...
                        @foreach (\App\Enums\Category::cases() as $cat)
                            <option value="{{ $cat->value }}" {{ old('category', $activity->category) == $cat->value ? 'selected' : '' }}>
                                {{ ucfirst($cat->value) }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="difficulty_level" name="difficulty_level"
                        ...
                        @foreach(['easy', 'moderate', 'hard'] as $level)
                            <option value="{{ $level }}" {{ old('difficulty_level', $activity->difficulty_level) == $level ? 'selected' : '' }}>
                                {{ ucfirst($level) }}
                            </option>
                        @endforeach
                    </select>
    ...
    <label class="custom-checkbox">
        <input type="checkbox" name="amenities[]" value="{{ $amenity }}"
            {{ collect(old('amenities', is_string($activity->amenities) ? json_decode($activity->amenities, true) : ($activity->amenities ?? [])))->contains($amenity) ? 'checked' : '' }}>
        <span>{{ $amenity }}</span>
    </label>
@endforeach
                    </div>
                    ...
                    <div class="flex items-center space-x-4 mb-2">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="requires_booking" value="1" class="hidden peer"
                                {{ old('requires_booking', $activity->requires_booking) == '1' ? 'checked' : '' }}>
                            <span
                        ...
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="requires_booking" value="0" class="hidden peer"
                                {{ old('requires_booking', $activity->requires_booking) == '0' ? 'checked' : '' }}>
                            <span
                    ...
                    <div class="flex items-center space-x-4 mb-2">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="pets_allowed" value="1" class="hidden peer"
                                {{ old('pets_allowed', $activity->pets_allowed) == '1' ? 'checked' : '' }}>
                            <span
                                class="px-3 py-1 rounded-lg border border-gray-300 bg-gray-100 text-gray-700 peer-checked:bg-green-500 peer-checked:text-white transition-all duration-200">
                                Yes
                            </span>
                        ...
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="pets_allowed" value="0" class="hidden peer"
                                {{ old('pets_allowed', $activity->pets_allowed) == '0' ? 'checked' : '' }}>
                            <span
                                class="px-3 py-1 rounded-lg border border-gray-300 bg-gray-100 text-gray-700 peer-checked:bg-red-500 peer-checked:text-white transition-all duration-200">
                                No
                            </span>
                    ...
                    <select id="family_friendly" name="family_friendly"
                        ...
                        <option value="{{ $option }}" {{ old('family_friendly', $activity->family_friendly) == $option ? 'selected' : '' }}>
                    ...
                    <textarea id="highlights" name="highlights"
                        class="block w-full border-gray-300 rounded-md shadow-sm">{{ old('highlights', $activity->highlights) }}</textarea>
